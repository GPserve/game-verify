// GanPlay Dragon Tiger 公平性驗證頁腳本
// 純瀏覽器端計算，演算法逐位元組對齊 mini_api services/seed_service.py::generate_dragontiger_result
// 與 enums/card.py 之 build_card_pool / encode_card / decode_card_suit / decode_card_rank。
//
// 每局固定 2 張生成牌（有放回、無實體副牌概念），與 baccarat 完全相同的
// sliding window 演算法，唯一差異是 i = 0..1。第 i 張：
...
//
// 牌流索引語意（僅供頁面呈現分組標籤，不影響任何一張牌的計算）：
//   index 0 = 龍（Dragon）；index 1 = 虎（Tiger）。
// 勝負比 rank：A(1) 最小 → K(13) 最大、花色不比、同 rank = 和（Tie）。
const GanDragonTiger = (() => {
  ...
  const computeDragonTigerResult = async (serverSeed, clientSeed, nonce) => {
    const cards = [];
    for (let i = 0; i < 2; i += 1) {
  ...
  return { computeDragonTigerResult, decodeCard, decodeCardParts };
})();
