...
const GanDice = (() => {
  ...
  const computeDiceResult = async (serverSeed, clientSeed, nonce) => {
    ...
    const rawFloat = Number(num) * Math.pow(2, -53) * 10000;
    const diceResult = rawFloat / 100;

    return diceResult.toFixed(2);
  };

  return { computeDiceResult };
})();
